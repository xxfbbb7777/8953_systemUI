...
public abstract class PanelView extends FrameLayout {
    ...
    {  
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,    
                float velocityY) {  
        	if(e2.getActionMasked()==MotionEvent.ACTION_UP || e2.getActionMasked()==MotionEvent.ACTION_CANCEL) {
        		 int pointerIndex = e2.findPointerIndex(mTrackingPointer);
                 if (pointerIndex < 0) {
                     pointerIndex = 0;
                     mTrackingPointer = e2.getPointerId(pointerIndex);
                 }
                 final float x = e2.getX(pointerIndex);
                 final float y = e2.getY(pointerIndex);
                 trackMovement(e2);
                 endMotionEvent(e2, x, y, false /* forceCancel */);           
                return true;  
        	}else {
        		return false;
        	}
