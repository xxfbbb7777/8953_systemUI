...
public class StatusBarIconController extends StatusBarIconList implements Tunable {
    ...
    private int blutoothViewIndex = -1;
    ...
    private void addSystemIcon(int index, StatusBarIcon icon) {
    	String pkg_name = "com.iflytek.inputmethod";
    	if(pkg_name.equals(icon.pkg)){
    		return;
    	}
    ...
    public void setIcon(String slot, int resourceId, CharSequence contentDescription) {
        ...
        if((mContext.getString(com.android.internal.R.string.status_bar_bluetooth)).equals(slot) && bluetoothIndex==-1){
    ...
    private void handleSet(int index, StatusBarIcon icon) {
        int viewIndex = getViewIndex(index);
        if( bluetoothIndex==index && blutoothViewIndex==-1 ){
        	blutoothViewIndex = viewIndex;
        }
        if(index==bluetoothIndex &&  blutoothViewIndex!=-1) 
        {
        	viewIndex =blutoothViewIndex;
        }
