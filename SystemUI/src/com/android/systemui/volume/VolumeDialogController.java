...
public class VolumeDialogController {
    ...
    private static final int MSG_STOP_SOUNDS = 102;
	...
	private boolean mVolumeSoundEnabled;
    ...
    public VolumeDialogController(Context context, ComponentName component) {
		...
		mVolumeSoundEnabled = mContext.getResources().getBoolean(com.android.internal.R.bool.config_useVolumeKeySounds);
    ...
    protected void onPlaySound(int streamType, int flags) {
        ...
        if (mWorker.hasMessages(MSG_STOP_SOUNDS)) {
            mWorker.removeMessages(MSG_STOP_SOUNDS);
        ...
        synchronized (this) {
            ...
            if (toneGen != null) {
                ...
                mWorker.sendMessageDelayed(mWorker.obtainMessage(MSG_STOP_SOUNDS), BEEP_DURATION);
            }
        }
    }
    ...
    private boolean onVolumeChangedW(int stream, int flags) {
		...
		final boolean playSound = ((flags & AudioManager.FLAG_PLAY_SOUND) != 0) && mVolumeSoundEnabled;
        boolean changed = false;
        if (showUI) {
		...
		if (playSound) {
            mWorker.removeMessages(MSG_PLAY_SOUND);
            mWorker.sendMessageDelayed(mWorker.obtainMessage(MSG_PLAY_SOUND, stream, flags), PLAY_SOUND_DELAY);
		}
        return changed;
    }
    ...
    private final class W extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
				...
				case MSG_PLAY_SOUND: onPlaySound(msg.arg1,msg.arg2); break;
				case MSG_STOP_SOUNDS: onStopSounds(); break;
